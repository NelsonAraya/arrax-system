        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>RRHH</h3>
                    <p class="text-subtitle text-muted">Asignacion de Material Mayor a Usuario</p>
                </div>
        ...
        <section class="section">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">Material Mayor Asignado a : {{ $usu->soloNombre() }}</h4>
                </div>
                <div class="card-content">
                    <div class="card-body">
                        <form class="form form-horizontal" method="POST" action="{{ route('rrhh.mat-mayor-update',$usu->id) }}">
                            @csrf
                            @method('PUT')
                            <div class="form-body">
                                <div class="row">
                                   ...
                                   <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" name="matMayor[]" id="_{{ $row->id }}" value="{{ $row->id }}" @if($flag) checked="checked" @endif>
                                        <label class="form-check-label" for="_{{ $row->id }}">{{ $row->clave }}</label>
                                    </div>
                                   @endforeach
